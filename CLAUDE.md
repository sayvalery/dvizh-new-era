...
### Правило: всегда использовать компоненты дизайн-системы

**При сборке любого компонента или страницы — СНАЧАЛА проверить, есть ли нужный элемент в UI Kit. Если есть — использовать его, а не писать аналог вручную.**

Это касается в первую очередь:
- **Кнопки** → всегда `Button.astro` (`variant`: `primary`, `secondary`, `ghost`, `dark`; `size`: `sm`, `md`, `lg`). Никаких самодельных `<a class="bg-brand ...">` или `<button class="...">`.
- **Инпуты** → всегда `Input.astro`.
- **Формы** → всегда `Form.astro`.
- **Любой другой компонент из `ui/`** — проверить перед тем, как писать с нуля.

Даже если в Figma кнопка выглядит иначе — подбираем ближайший `variant` из `Button.astro`. Если ни один вариант не подходит — сначала **расширяем `Button.astro`**, а не создаём отдельный элемент.
