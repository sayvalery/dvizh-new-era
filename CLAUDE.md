...
### Правило: токены Figma → дизайн-система

**Если значение из Figma не совпадает точно с токеном — брать ближайший токен из дизайн-системы. Не вводить новые произвольные значения.**

#### Размеры шрифтов → типографические классы (`global.css`)
...
| Inter, 14–16px, w400 | `.text-subhead` |
| Inter, 14px, w400 | `.text-footnote` |

> Заголовочные теги (`h1`–`h6`) уже получают `letter-spacing` из глобального CSS — не дублировать `tracking-*` поверх них.

#### Цвета → Tailwind-токены (`tailwind.config.ts`)

| Figma | Токен |
|-------|-------|
| `#FBF9F6` (cream bg) | `bg-gray-100` |
| `#F9F7F5` | `bg-background-secondary` |
| `#1D1E21` / тёмный текст | `text-gray-900` |
| `#a8a8a8` (muted) | `text-gray-400` |
| `#858585` | `text-gray-500` |
| `#ff4d00` (бренд) | `text-brand`, `bg-brand` |

#### Исключения (допустимо использовать произвольные значения)

- **CSS-градиенты** — `linear-gradient(...)` не поддерживает Tailwind-токены, inline `style` допустим. Цвет в градиенте брать максимально близкий к токену (`#ff4d00` вместо `#FF600A`).
- **Уникальные значения отступов** — `py-[120px]` допустимо, если в шкале Tailwind нет точного значения и это задано явно в Figma.
- **Явно согласованные отклонения** — фиксировать комментарием в коде.
...
```
master  ← ПРОД. Деплой собирает из master. preview→dvizh.io. Обновляется ТОЛЬКО промоутом dev→master (Valery)
...
В `master` напрямую НЕ коммитить. В `dev` напрямую НЕ коммитить — только через мерж из своей ветки.

**Целевая модель деплоя:** `master` = прод, деплой пересобирает сайт **из `master`**. Мерж в `dev` попадает **только на preview**, в прод сам по себе НЕ идёт. Выкатка в прод = осознанный промоут `dev→master` (см. ниже).

> ⚠️ **Сейчас деплой ФАКТИЧЕСКИ собирает из `dev`** (`scripts/build-site.sh`, `SOURCE_BRANCH="dev"`). Переключение на `master` — **pending (Фаза 3** миграции в GitLab, см. `docs/superpowers/specs/2026-06-18-gitlab-migration-design.md`). Порядок критичен: сначала промоут `dev→master`, только потом смена `SOURCE_BRANCH`. До переключения модель «master = прод» — целевая, не действующая.

### Рабочий процесс

- **Начало** — скилл `sync` (или «начни работу») → переключает на ветку, rebase на `dev`
- **Публикация** — «опубликуй» → мерж личной ветки в `dev`, push. Так коллеги забирают изменения
- **Что показывает preview** — `preview.dvizh.cc` это dev-сервер в рабочем каталоге на **той ветке, которая там выкачена**, а НЕ ветка `dev`. Мерж в `dev` сам по себе на preview не влияет; чтобы показать свою ветку — достаточно её выкатить локально
- **Выкатка в прод** — «выкатить в прод» (Режим 3 `sync`, гейт по Valery) → промоут `dev→master` (мерж + push) → деплой из `master`. Это **единственный** путь в прод
- **Контент-публикация** — кнопка «Опубликовать» в CMS (`POST /api/deploy` → вебхук на порту 3099 → `build-site.sh`) — **живой триггер прод-деплоя**, доступный любому залогиненному в админке. Пока `SOURCE_BRANCH="dev"`, она собирает прод **из `dev`**: всё влитое в `dev` уедет на dvizh.cc при первой же контент-публикации. После Фазы 3 будет собирать неизменный `master`, и эксперименты из `dev` перестанут утекать
...
bash scripts/build-site.sh   # Сборка + деплой (рестарт nginx). Сейчас собирает из dev (Фаза 3 → master pending)
