...
export const MonitorStatus: GlobalConfig = {
  slug: 'monitor-status',
  label: 'Данные статус-страницы (служебное)',
  admin: {
    // Скрыто из левой навигации: статус-страница (/admin/status) остаётся, но её «настройка»
    // (этот глобал) настраивается один раз и не маячит в меню. Монитор пишет сюда через API.
    hidden: true,
